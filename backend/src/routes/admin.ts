...
router.post('/branches/:id/fund', authMiddleware, async (req: AuthRequest, res) => {
    try {
        const admin = await prisma.staff.findUnique({ where: { id: req.userId }, select: { id: true, name: true, role: true, isActive: true, permissions: true, permissionsCustomized: true, branchId: true } });
        // Correction : ceci vérifiait 'perm_audit_log_view' ("consulter les logs d'audit"), une
        // permission accordée par défaut à COMPLIANCE_CHECKER — lui donnant de facto le pouvoir
        // de déplacer de la liquidité physique HQ→agence sans passer par le circuit
        // Maker/Checker de treasury.ts. 'perm_treasury_allocate' ("Allouer des fonds à une
        // agence") est la permission qui correspond réellement à cette action, et n'est
        // accordée qu'aux rôles ayant l'ensemble des droits (SUPER_ADMIN/ADMIN).
        if (!admin || !hasPermission(admin, 'perm_treasury_allocate')) {
...
router.get('/cards/all', authMiddleware, async (req: AuthRequest, res) => {
    try {
        ...
        // `include: { user: true }` récupérait l'objet User complet — dont les 3 photos KYC en
        // base64 — pour chaque carte listée ici, gardé seulement par perm_analytics_view
        // (accordée par défaut à BRANCH_MANAGER). `select` limite à ce que la page affiche réellement.
        const cards = await prisma.virtualCard.findMany({ include: { user: { select: { name: true, phone: true } } }, orderBy: { id: 'desc' }, take: 100 });
        res.json(cards);
    } catch (e: any) {
        res.status(500).json({ error: friendlyErrorMessage(e) });
    }
...
// PUT /api/admin/cards/:id/freeze — le bouton "Gel d'Urgence" de VirtualCards.tsx n'appelait
// jusqu'ici aucune route : PUT /api/wallet/cards/:id/freeze (cards.ts) exige que le titulaire
// authentifié soit le propriétaire de la carte, donc inutilisable par le personnel pour geler
// la carte d'un client depuis le back-office. Même permission que le gel de compte client
// (perm_customer_freeze), puisqu'il s'agit du même type d'action de confinement d'urgence.
router.put('/cards/:id/freeze', authMiddleware, async (req: AuthRequest, res) => {
    try {
        const admin = await prisma.staff.findUnique({ where: { id: req.userId } });
        if (!admin || !hasPermission(admin, 'perm_customer_freeze')) {
            return res.status(403).json({ error: 'Autorisation "Geler un compte/instrument client" requise.' });
        }
